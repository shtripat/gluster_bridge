...
sys.modules['tendrl.gluster_bridge.atoms.volume.log_rotate'] = MagicMock()
sys.modules['etcd'] = MagicMock()
from tendrl.gluster_bridge.flows.volume_log_rotate import VolumeLogRotate


class TestVolumeLogRotate(object):

    def test_start(object):
        flow = VolumeLogRotate(
            {
                ...
                "object_type": "volume",
                "status": 'new',
                "message": 'Rotating logs for volume',
                "attributes": {
                    "volname": 'Volume1',
                },
                "errors": {}
