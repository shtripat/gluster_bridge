...
from tendrl.gluster_integration.objects.volume.atoms import volume_exists


class TestVolumeExists(object):
    def test_run(self, monkeypatch):
        self.obj = volume_exists.VolumeExists(
            "volume_exists",
            True,
            "Volume exists",
            ["Volume.vol_id"],
            [],
            "242f6190-9b37-11e6-950d-a24fc0d9653c"
        )
        volume_exists.volume.etcd = MagicMock()
        volume_exists.volume.TendrlConfig = MagicMock()
        volume_exists.volume.brick.etcd = MagicMock()
        volume_exists.volume.brick.TendrlConfig = MagicMock()
        volume_ins = volume_exists.volume.Volume("", "volumeid1", "clusterid1")
        brick_ins = volume_exists.volume.brick.Brick(
            "clusterid1",
            "volumeid1",
            "host1:brick1"
        )
