...
from tendrl.gluster_integration.objects.volume.atoms import volume_stopped


class TestVolumeStopped(object):
    def test_run(self, monkeypatch):
        self.obj = volume_stopped.VolumeStopped(
            "Stop",
            True,
            "Stop Volume",
            ["Volume.volname"],
        ...
        volume_stopped.volume.etcd = MagicMock()
        volume_stopped.volume.TendrlConfig = MagicMock()
        volume_stopped.volume.brick.etcd = MagicMock()
        volume_stopped.volume.brick.TendrlConfig = MagicMock()
        volume_ins = volume_stopped.volume.Volume(
            "",
            "volumeid1",
            "clusterid1"
        )
        brick_ins = volume_stopped.volume.brick.Brick(
            "clusterid1",
            "volumeid1",
            "host1:brick1"
        )
